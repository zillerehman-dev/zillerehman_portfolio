...
export async function POST(request) {
  try {
    ...
    const emailPasskey = process.env.GMAIL_PASSKEY;

    const hasTelegram = Boolean(token && chat_id);
    const hasEmail = Boolean(emailAddress && emailPasskey);

    if (!hasTelegram && !hasEmail) {
      return NextResponse.json({
        success: false,
        message: "Telegram and email config missing.",
      }, { status: 400 });
    }
